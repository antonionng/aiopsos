...
(async()=>{
 ...
 const campaign='/api/assessment/'+assessmentIds[0];
 assert.equal((await request(owner,campaign+'/campaign')).status,200);
 assert.equal((await request(foreign,campaign+'/campaign')).status,404);
 assert.equal((await request(learner,campaign+'/campaign')).status,403);
 ...
 const checks=[['/api/assessment','GET'],['/api/assessment','POST'],['/api/assessment','DELETE'],[campaign+'/campaign','GET'],[campaign+'/invite','POST'],[campaign+'/invite/csv','POST'],[campaign+'/remind','POST'],[campaign+'/link','GET'],[campaign+'/responses/'+crypto.randomUUID(),'DELETE']];
 for(const [route,method] of checks){const r=await fetch(base+route,{method,headers:{Cookie:owner.header,'Content-Type':'application/json'},...(method==='GET'?{}:{body:'{}'})});assert.equal(r.status,403,method+' '+route);}
 console.log('PASS: own/foreign/learner campaign boundaries and revoked list/create/delete/invite/CSV/remind/link/response deletion. No emails sent.');
