...
export async function POST(req: NextRequest) {
  ...
  // The SELECT policy checks the stored cohort through a STABLE helper.
  // Read in a separate statement so that helper can see the committed insert.
  const { data: stored } = await supabase
    .from("cohorts")
    .select(COHORT_COLUMNS)
    .eq("id", cohortId)
    .maybeSingle();
  const data = stored ?? { ...input, id: cohortId, org_id: actor.orgId };
