-- Prevent browser profile edits from changing authorisation identity.
-- Service-side administration remains available. Super-admin tenant switching
-- is retained, while ordinary people can still edit their descriptive profile.
create or replace function public.guard_profile_authorisation() returns trigger
language plpgsql security invoker set search_path=public,pg_temp as $$
begin
 if current_user in ('authenticated','anon') then
  if new.id is distinct from old.id or new.role is distinct from old.role or new.email is distinct from old.email or new.plan_override is distinct from old.plan_override or ((new.org_id is distinct from old.org_id or new.department_id is distinct from old.department_id) and old.role<>'super_admin') then
   raise exception 'Workspace membership and access are managed by an administrator' using errcode='42501';
  end if;
 end if;
 return new;
end; $$;
create trigger zz_guard_profile_authorisation before update on public.user_profiles for each row execute function public.guard_profile_authorisation();
revoke all on function public.guard_profile_authorisation() from public,anon,authenticated;
...
begin
 ...
 if p_action in ('overview','course.get','programme.get','learning.get','record.get') then null;
 else
  ...
  if saved.id is not null then
   if saved.org_id<>actor.org_id then raise exception 'Request belongs to a different workspace' using errcode='42501'; end if;
 ...
 if p_action='overview' then
  return jsonb_build_object('org_id',actor.org_id,'can_manage',manager,
   ...
   'programmes',coalesce((select jsonb_agg(p order by p.created_at desc) from lms_programmes p where (manager and (p.org_id=actor.org_id or p.client_org_id=actor.org_id)) or exists(select 1 from lms_assignments a where a.programme_id=p.id and a.user_id=p_actor and a.org_id=actor.org_id)),'[]'),
   'assignments',coalesce((select jsonb_agg(to_jsonb(a)||jsonb_build_object('title',p.title,'due_at',p.due_at,'learner_name',u.name)) from lms_assignments a join lms_programmes p on p.id=a.programme_id join user_profiles u on u.id=a.user_id where (a.user_id=p_actor and a.org_id=actor.org_id) or (manager and (p.org_id=actor.org_id or a.org_id=actor.org_id))),'[]'),
 ...
 elsif p_action='record.get' then
  select a.* into assignment from lms_assignments a join lms_programmes p on p.id=a.programme_id where a.id=(p_payload->>'id')::uuid and (p.org_id=actor.org_id or a.org_id=actor.org_id);
  if assignment.id is null then raise exception 'Record not found' using errcode='P0002'; end if;
  select * into prog from lms_programmes where id=assignment.programme_id;
  return jsonb_build_object('assignment',to_jsonb(assignment),'programme',to_jsonb(prog),'activities',(select jsonb_agg(jsonb_build_object('id',a->>'id','title',a->>'title','kind',a->>'kind','criteria',a->>'criteria','course_title',v.content->>'title','version',v.version)) from lms_course_versions v cross join lateral jsonb_array_elements(v.content->'activities') a where v.id=any(prog.version_ids)), 'history',coalesce((select jsonb_agg(h order by h.created_at) from lms_attempt_history h where h.assignment_id=assignment.id),'[]'));
 elsif p_action='client.disconnect' then
  update lms_clients set state='declined',responded_by=p_actor where id=(p_payload->>'id')::uuid and actor.org_id in(provider_org_id,client_org_id) and state='accepted' returning id into record_id;
  if record_id is null then raise exception 'Active connection not found' using errcode='P0002'; end if;
...
begin
 ...
 if r.state='running' and r.lease_until<now() and r.attempts>=3 then update lms_agent_runs set state='failed',error='Retry limit reached after interrupted execution',lease_token=null,lease_until=null,updated_at=now() where id=p_id; return null; end if;
